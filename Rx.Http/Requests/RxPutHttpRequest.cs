...
namespace Rx.Http.Requests
{
    internal class RxPutHttpRequest : RxHttpRequest
    {
        ...
        protected override Task<HttpResponseMessage> ExecuteRequest(string url, HttpContent content, CancellationTokenSource cancellationTokenSource) => http.PutAsync(url, content, cancellationTokenSource.Token);
    }
}
